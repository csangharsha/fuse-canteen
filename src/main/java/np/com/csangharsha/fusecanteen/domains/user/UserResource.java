...
public class UserResource extends BaseResource<User, UserDto> {
    ...
    public ResponseEntity<List<OrderDto>> getPendingOrders(@PathVariable("id") Long id) throws AccessDeniedException {
        Long currentLoggedInUserId = ((UserPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getId();
        if(!currentLoggedInUserId.equals(id)) {
    ...
    public ResponseEntity<List<OrderDto>> getOrderHistoryOf(@PathVariable("id") Long id) throws AccessDeniedException {
        Long currentLoggedInUserId = ((UserPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getId();
        if(!currentLoggedInUserId.equals(id)) {
            throw new AccessDeniedException("Not Authorized.");
        }
